...
for (i in 1:6) {
  ...
  data <- data.frame(
    x = plot(date_model$model, trans = plogis,
             shift = coef(date_model$model)[1], se = 1.96,
             seWithMean = TRUE)[[i]]$x,
    fit = plot(date_model$model, trans = plogis,
               shift = coef(date_model$model)[1], se = 1.96,
               seWithMean = TRUE)[[i]]$fit,
    se = plot(date_model$model, trans = plogis,
              shift = coef(date_model$model)[1], se = 1.96,
              seWithMean = TRUE)[[i]]$se, 
    intercept = rep(coef(date_model$model)[1], 100)
  )
...
snowhight_smooth <- plot_list[[6]] +
                labs(title = "glatte Funktion für Neuschnee",
                     x = "Neuschnee in cm",
                     y = "s(Neuschnee)") +
                theme(plot.title = element_text(hjust = 0.5)) +
